...
| `src/providers/uefa/__tests__/index.test.ts`                | UEFA provider: paginationMode, getSeasons() descending order, seasonLabel format, getExternalUrl; `fetchMatches()` maps raw `minute: {normal: N}` → plain number, handles missing minute (FINISHED), handles plain-number passthrough; `fetchMatch()` maps `minute: {normal: N}` at 90 and 45, missing minute → undefined, preserves all other fields; `fetchMatchLineups()` pass-through correctness                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `src/providers/fifa/__tests__/fifa.test.ts`                 | FIFA World Cup provider: id/paginationMode/defaultDisplayMode metadata, getSeasons/getDefaultSeason/seasonLabel(2022), getTotalGameweeks=8, getDefaultGameweek=8; stageToGameweek for all 8 rounds (MD1/2/3, R16, QF, SF, 3rd-place, Final); fetchMatches/fetchMatchesByGameweek (string IDs, score only on FINISHED, round filtering, unknown season throws); fetchMatch (team names, score with penalties, stadium, round, UPCOMING has no score); fetchMatchLineups: coord normalization (home GK y=0, away GK y=1000, all coords integers in [0,1000]), 11 starters+bench per team, HIT/PARTIAL/MISS club enrichment, penalty shootout (penaltyScorers from Period=11 goals), error handling (404/500); fixture shape assertions; **multi-year support**: FIFA_SEASON_IDS[2018]='254645', getSeasons()=[2022,2018], getDefaultSeason()=2022 regression, seasonLabel(2018)='Russia 2018', seasonLabel(2022) regression; **fetchMatchLineups year-switching**: 2018 match (IdSeason='254645') consults 2018 squad map (Griezmann→'Atletico Madrid 2018'), 2022 match unaffected (Messi→'Inter Miami'); **getExternalUrl year-aware**: 2022 date→'255711', 2018 date→'254645', unknown year falls back to '255711' |

### Component Tests — Match

| File                                                       | What it tests                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/components/match/__tests__/MatchCard.test.tsx`        | Team names/logos, per-row scores, penalty scores, round info, routing links (string + number IDs); status states: FT+Today (fake clock), FT+past date, Live+minute (aria-label), Live without minute, Tomorrow+time (fake clock), future date+time, upcoming no score; winner `◄` indicator (home win, away win, draw, upcoming, live draw); aggregate strip (SECOND_LEG with/without data, FIRST_LEG absent); YouTube highlights link (past finished, today finished absent, upcoming absent, live absent, URL contains team names + competition); edge cases: no extra info separator, string match IDs; red card indicators: no playerEvents, empty redCards array, home team 1 card, away team 1 card, home team multiple cards (count + ×N text), both teams have cards, singular/plural aria-label (`"1 red card"` vs `"2 red cards"`) |
...
| `src/pages/__tests__/MatchDetailPage.test.tsx` | Match detail page rendering with lineup data. Live indicator: shows pulsing "Live" badge (emerald-500) when status is LIVE, hidden when FINISHED. **FIFA World Cup describe block**: team names/score, player name visible, HIT player club logo img rendered, MISS player flag fallback, 404 error message. |
...
| `src/test/msw/handlers.ts` | HTTP GET handlers for UEFA (`/uefa-api/v5/*`), Ligue 1 (`/ligue1-api/*`), Premier League (`/pl-api/*`), and FIFA (`/fifa-api/*`) API proxy routes                                                                                                                                                                                                                                             |
| `src/test/msw/fixtures.ts` | Test data — UEFA: Real Madrid vs Barcelona match, Bayern vs PSG match, lineup data; Ligue 1: PSG vs Toulouse match/lineups/standings; Premier League: Bournemouth vs Man Utd match/lineups/events/teams/squad; FIFA: calendar (3 matches across GW1/4/8) + full Final match detail (Argentina vs France, 11 starters+3 bench per team, penalty shootout, HIT/PARTIAL/MISS enrichment players) |
